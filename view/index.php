...
<body>
  ...
  <script>
    ...
    // プロフィールデータの取得
    const JSON_user = <?= $result_json_users; ?>;
    const JSON_user1 = JSON.stringify(JSON_user);
    const user = JSON.parse(JSON_user1);
    console.log(user);
    // 職種データの取得
    const JSON_oc = <?= $result_json_occupations; ?>;
    const JSON_oc1 = JSON.stringify(JSON_oc);
    const oc = JSON.parse(JSON_oc1);
    console.log(oc);
    // 使用可能言語データの取得
    const JSON_lang = <?= $result_json_langs; ?>;
    const JSON_lang1 = JSON.stringify(JSON_lang);
    const la = JSON.parse(JSON_lang1);
    console.log(la);
  </script>
</body>
